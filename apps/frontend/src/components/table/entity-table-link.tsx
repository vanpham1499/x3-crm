...
type EntityTableButtonProps = Omit<EntityTableLinkProps, 'href'> & {
  onClick: () => void;
  ariaLabel?: string;
};
...
export function EntityTableButton({
  children,
  title,
  tone = 'primary',
  className = '',
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={`block max-w-full cursor-pointer truncate text-left font-bold transition-colors focus-visible:rounded-sm focus-visible:outline-none focus-visible:ring-2 ${toneClassNames[tone]} ${className}`}
    >
      {children}
    </button>
  );
