...
export function LeadManager({
  ...
  const statusOptions = useMemo(() => getUniqueLeadStatuses(leads), [leads]);
  ...
  const isAllVisibleSelected = visibleLeadIds.length > 0 && selectedVisibleCount === visibleLeadIds.length;
  ...
  return (
    <div className="min-h-[calc(100vh-72px)] w-full bg-slate-50/60 p-6">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button component={Link} href="/leads/new" variant="contained" startIcon={<AddRoundedIcon />} className="!bg-slate-900 hover:!bg-slate-800">
          Thêm lead
        </Button>
      </div>
      ...
      <section className="w-full overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="grid gap-3 border-b border-slate-200 p-5 lg:grid-cols-[minmax(280px,1fr)_repeat(4,190px)]">
          ...
          <TextField select label="Trạng thái" value={filters.status_id} onChange={(event) => updateFilters({ status_id: event.target.value })}>
          ...
          <TextField select label="Nhân sự" value={filters.assigned_user_id} onChange={(event) => updateFilters({ assigned_user_id: event.target.value })}>
            <MenuItem value="">Tất cả</MenuItem>
            {users.map((user) => (
              <MenuItem key={user.id} value={user.id}>{user.name || user.email || user.code}</MenuItem>
            ))}
          </TextField>

          <TextField select label="Nguồn" value={filters.source_id} onChange={(event) => updateFilters({ source_id: event.target.value })}>
            <MenuItem value="">Tất cả</MenuItem>
            {sources.map((source) => (
          ...
          <TextField select label="Dịch vụ" value={filters.interested_service_id} onChange={(event) => updateFilters({ interested_service_id: event.target.value })}>
            <MenuItem value="">Tất cả</MenuItem>
            {serviceOptions.map((service) => (
              <MenuItem key={service.id} value={service.id}>{'— '.repeat(service.depth)}{service.name}</MenuItem>
            ))}
          </TextField>
        </div>
        ...
        {hasSelectedRows && (
          <div className="flex h-14 items-center justify-between bg-emerald-100 px-5 text-sm font-bold text-emerald-700">
            <div className="flex items-center gap-4">
              <Checkbox color="success" size="small" checked onChange={(event) => toggleAllVisibleRows(event.target.checked)} />
              <span>{selectedLeadIds.length} selected</span>
            </div>
            <IconButton size="small" color="success" onClick={() => setSelectedLeadIds([])} title="Xóa lựa chọn">
        ...
        <div className="relative w-full overflow-x-auto">
          ...
          <table className={`w-full min-w-[1280px] table-fixed text-left text-sm transition-opacity ${isFetching ? 'opacity-60' : 'opacity-100'}`}>
            <thead className="bg-slate-50 text-xs font-bold uppercase text-slate-500">
              <tr>
                <th className="sticky left-0 z-20 w-12 bg-slate-50 px-5 py-4">
                  <Checkbox color="success" size="small" checked={isAllVisibleSelected} indeterminate={isSomeVisibleSelected} onChange={(event) => toggleAllVisibleRows(event.target.checked)} />
                </th>
                ...
                <th className="w-32 px-3 py-4">Ngày chốt</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {leads.length === 0 ? (
                ...
                leads.map((lead) => {
                  ...
                  const serviceName = lead.interestedService?.name || lead.interestedServiceText || '-';

                  return (
                    <tr key={lead.id} className={`group ${isSelected ? 'bg-emerald-50/60' : 'hover:bg-slate-50/80'}`}>
                      <td className={`sticky left-0 z-10 px-5 py-3 ${isSelected ? 'bg-emerald-50/60' : 'bg-white group-hover:bg-slate-50'}`}>
                        <Checkbox color="success" size="small" checked={isSelected} onChange={(event) => toggleLeadRow(lead.id, event.target.checked)} />
                      </td>
                      <td className={`sticky left-12 z-10 px-3 py-3 ${isSelected ? 'bg-emerald-50/60' : 'bg-white group-hover:bg-slate-50'}`}>
                        <div className="w-[336px] rounded-xl border border-slate-100 bg-white p-3 shadow-sm group-hover:border-slate-200">
                          <div className="flex min-w-0 items-center gap-2">
                            <span className="shrink-0 rounded-md bg-slate-100 px-2 py-1 text-[11px] font-bold text-slate-700">{lead.leadCode || '-'}</span>
                            <p className="truncate font-semibold text-slate-950" title={lead.customerName}>{lead.customerName}</p>
                          </div>
                          ...
                          <div className="mt-2 flex min-w-0 flex-wrap items-center gap-2">
                            ...
                            {lead.industry && (
                              <span className="rounded-md bg-slate-100 px-1.5 py-0.5 text-[11px] font-semibold text-slate-600">{lead.industry}</span>
                            )}
                            {link && (
                              <a href={externalUrl(link)} target="_blank" rel="noreferrer" className="inline-flex min-w-0 items-center gap-1 rounded-md bg-blue-50 px-1.5 py-0.5 text-[11px] font-bold text-blue-700 ring-1 ring-blue-100 hover:bg-blue-100" title={link}>
                      ...
                      <td className="px-3 py-4">
                        <span className={`rounded-md px-2 py-1 text-xs font-bold ring-1 ${getLeadStatusClass(lead.status)}`}>
                          {lead.status?.name || 'Mới'}
                        </span>
                      </td>
                      <td className="px-3 py-4">
                      ...
                      <td className="px-3 py-4">
                        <span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-bold text-slate-700 ring-1 ring-slate-200" title={serviceName}>
                          {serviceName}
                        </span>
                      </td>
                      <td className="px-3 py-4 text-slate-700">{formatDate(lead.occurredDate || '')}</td>
                      <td className="px-3 py-4 text-slate-700">{formatDate(lead.closedDate || '')}</td>
                      <td className="px-3 py-4">
                        <p className="line-clamp-2 max-w-[420px] text-slate-500" title={lead.note || ''}>{lead.note || '-'}</p>
                      </td>
                      <td className="py-4">
                        <div className="flex items-center justify-end gap-1 pr-3">
                          <IconButton component={Link} href={`/leads/${lead.id}`} size="small" title="Chỉnh sửa lead">
                            <EditRoundedIcon fontSize="small" />
                          </IconButton>
                          <IconButton size="small" title="Tác vụ" onClick={(event) => openActionMenu(event, lead)}>
