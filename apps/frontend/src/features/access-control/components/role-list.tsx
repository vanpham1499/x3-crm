      ...
      <section className="w-full overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="p-4">
          <CompactSearchField
            ...
            onChange={(keyword) => onFiltersChange({ ...filters, keyword })}
          />
        </div>
        ...
        >
          {roles.map((role) => (
            <tr key={role.id} className="group hover:bg-slate-50/80">
        ...
        <div className="border-t border-slate-200 px-5 py-4 text-sm text-slate-500">
          Hiển thị <strong className="text-slate-950">{roles.length}</strong> vai trò
        </div>
      </section>
